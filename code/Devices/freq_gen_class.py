...
class chActFreqGen(base_ch):
    def __init__(
        ...
        self.min_step_F = min_step_F
        self.min_step_A = min_step_A
        self.dict_buf_parameters["type_of_work"] = "Изменение частоты"
        self.dict_buf_parameters["type_step"] = "Заданный шаг"
        self.dict_buf_parameters["high_limit"] = str(5)
        self.dict_buf_parameters["low_limit"] = str(self.min_step_F)
...
class FreqGen(base_device):
    ...
    def __init__(self, name, type_connection, installation_class) -> None:
        ...
        self.setting_window.radioButton.setToolTip(
            "При активации этого пункта генератор частоты пройдет по шагам от стартого значения до конечного и обратно."
        )
