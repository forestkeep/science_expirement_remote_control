...
def process_and_export(input_file_path, output_file_path, output_type):
    ...
    def export_to_excel(instruments_data, output_file_path):
        ...
        df = pd.DataFrame.from_dict({(instrument_name, key): measurements_list for instrument_name, measurements_dict in instruments_data.items() for key, measurements_list in measurements_dict.items()}, orient='index')
        df.index = pd.MultiIndex.from_tuples(df.index)
        df.index.names = ['Instrument', 'Measurement']
...
if __name__ == "__main__":
    process_and_export(fr"C:\Users\asus\Desktop\remake\Maisheng1_Lock in2_2024-02-27 18-26-10.txt", fr"C:\Users\asus\Desktop\remake\результаты.xlsx", 'excel')