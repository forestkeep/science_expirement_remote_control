...
class power_supply(base_device):
    def __init__(self, name, type_connection, installation_class) -> None:
            ...
            self.setting_window.type_work_enter.addItems(["Стабилизация напряжения", "Стабилизация тока"])
            self.setting_window.type_step_enter.addItems(["Заданный шаг"])
            self.setting_window.type_step_enter.setStyleSheet(ready_style_border)
            self.setting_window.type_work_enter.setStyleSheet(ready_style_border)
            # =======================прием сигналов от окна==================
            self.setting_window.type_work_enter.currentIndexChanged.connect(lambda: self._change_units())
            self.setting_window.type_work_enter.currentIndexChanged.connect(lambda: self._is_correct_parameters())
            self.setting_window.type_step_enter.currentIndexChanged.connect(lambda: self._action_when_select_step())
            self.setting_window.type_step_enter.currentIndexChanged.connect(lambda: self._is_correct_parameters())
            self.setting_window.type_step_enter.currentIndexChanged.connect(lambda: self._action_when_select_trigger())
            self.setting_window.type_step_enter.setToolTip("Доступен фиксированный шаг. Адаптивный находится в разработке.")
            self.setting_window.step_enter.currentTextChanged.connect(lambda: self._is_correct_parameters())
            self.setting_window.stop_enter.currentTextChanged.connect(lambda: self._is_correct_parameters())
            self.setting_window.start_enter.currentTextChanged.connect(lambda: self._is_correct_parameters())
            self.setting_window.second_limit_enter.currentTextChanged.connect(lambda: self._is_correct_parameters())
    ...
    def show_setting_window(self, number_of_channel):
            ...
            self.setting_window.type_work_enter.setCurrentText(self.active_channel_act.dict_buf_parameters["type_of_work"])
            self.setting_window.type_step_enter.setCurrentText(self.active_channel_act.dict_buf_parameters["type_step"])
            self.setting_window.start_enter.setCurrentText(str(self.active_channel_act.dict_buf_parameters["low_limit"]))
            self.setting_window.stop_enter.setCurrentText(str(self.active_channel_act.dict_buf_parameters["high_limit"]))
            self.setting_window.step_enter.setCurrentText(str(self.active_channel_act.dict_buf_parameters["step"]))
            self.setting_window.second_limit_enter.setCurrentText(str(self.active_channel_act.dict_buf_parameters["second_value"]))
            ...
            if (self.active_channel_act.dict_buf_parameters["type_of_work"]== "Стабилизация напряжения"):
                self.setting_window.second_value_limit_label.setText("Ток не выше (А)")

            elif (self.active_channel_act.dict_buf_parameters["type_of_work"]== "Стабилизация тока"):
                self.setting_window.second_value_limit_label.setText("Напряжение не выше (V)")
            else:
                self.setting_window.second_value_limit_label.setText("---")
                self.setting_window.second_limit_enter.setEnabled(False)

            self.setting_window.voltage_meas.setChecked(self.active_channel_meas.dict_buf_parameters["meas_vol"] == True)
            self.setting_window.current_meas.setChecked(self.active_channel_meas.dict_buf_parameters["meas_cur"] == True)
            self.setting_window.set_voltage_meas.setChecked(self.active_channel_meas.dict_buf_parameters["meas_set_vol"] == True)
            self.setting_window.set_current_meas.setChecked(self.active_channel_meas.dict_buf_parameters["meas_set_cur"] == True)
    ...
    def _is_correct_parameters(self):  # менять для каждого прибора
        if self.key_to_signal_func:
            ...
            try:
                second_limit = float(self.setting_window.second_limit_enter.currentText())
            except:
            ...
            if self.active_channel_act.is_step_correct:
                if (self.active_channel_act.is_start_value_correct and self.active_channel_act.is_stop_value_correct):
                    if enter_step > abs(high_value - low_value):
                        self.active_channel_act.is_step_correct = False

            if (self.active_channel_act.is_second_value_correct and self.setting_window.type_work_enter.currentText() != "Стабилизация мощности"):
            ...
            if self.active_channel_act.is_step_correct:
                if (self.setting_window.type_step_enter.currentText()== "Адаптивный шаг"):
                    self.setting_window.step_enter.setStyleSheet("background-color: rgb(180, 180, 180)")
                else:
            ...
            if self.active_channel_act.is_second_value_correct:
                ...
                if (self.setting_window.type_work_enter.currentText()!= "Стабилизация мощности"):
                    self.setting_window.second_limit_enter.setStyleSheet(not_ready_style_border)
                else:
                    self.setting_window.second_limit_enter.setStyleSheet(ready_style_border)


            return (    self.active_channel_act.is_stop_value_correct \
    ...
    def _change_units(self):
        if self.key_to_signal_func:
            ...
            if (
                self.setting_window.type_work_enter.currentText()== "Стабилизация мощности"):
    ...
    def add_parameters_from_window(self):  # менять для каждого прибора
        ...
        if self.key_to_signal_func:
            self.active_channel_act.dict_buf_parameters["type_of_work"] = (self.setting_window.type_work_enter.currentText())
            self.active_channel_act.dict_buf_parameters["type_step"] = (self.setting_window.type_step_enter.currentText())
            self.active_channel_act.dict_buf_parameters["high_limit"] = (self.setting_window.stop_enter.currentText())
            self.active_channel_act.dict_buf_parameters["low_limit"] = (self.setting_window.start_enter.currentText())
            self.active_channel_act.dict_buf_parameters["step"] = (self.setting_window.step_enter.currentText())
            
            self.active_channel_act.dict_buf_parameters["second_value"] = (self.setting_window.second_limit_enter.currentText())

            self.active_channel_act.dict_buf_parameters["repeat_reverse"] = self.setting_window.radioButton.isChecked()
            self.active_channel_act.dict_buf_parameters["soft_start"] = self.setting_window.is_soft_start.isChecked()
            self.active_channel_act.dict_buf_parameters["soft_off"] = self.setting_window.is_soft_stop.isChecked()

            self.active_channel_meas.dict_buf_parameters["meas_cur"] = self.setting_window.current_meas.isChecked()
            self.active_channel_meas.dict_buf_parameters["meas_vol"] = self.setting_window.voltage_meas.isChecked()
            self.active_channel_meas.dict_buf_parameters["meas_set_cur"] = self.setting_window.set_current_meas.isChecked()
            self.active_channel_meas.dict_buf_parameters["meas_set_vol"] = self.setting_window.set_voltage_meas.isChecked()
    ...
    def send_signal_ok(self,):
        ...
        if (self.active_channel_act.dict_buf_parameters == self.active_channel_act.dict_settable_parameters and self.dict_buf_parameters == self.dict_settable_parameters):
            #return
            pass
    ...
    def confirm_parameters(self):  # менять для каждого прибора
        ...
        for ch in self.channels:
            ...
            if (
                ...
                if (
                    ...
                    (
                        ...
                        float(
                            self.active_channel_act.dict_settable_parameters["low_limit"]
                        ),
                        float(
                            self.active_channel_act.dict_settable_parameters["high_limit"]
                        ),
                        float(
                            self.active_channel_act.dict_settable_parameters["step"]),
                        float(
                            self.active_channel_act.dict_settable_parameters["second_value"]
                        ),
                    )
            ...
            self.active_channel_act.dict_settable_parameters["num steps"] = int(len(self.active_channel_act.steps_voltage))
    ...
    def do_action(self, ch, repeat = 3) -> bool:
        ...
        if ch.get_type() == "act":
            if ch.step_index < len(ch.steps_voltage) - 1:
                if ch.step_index == 0 and ch.dict_settable_parameters["soft_start"] == True:
                    print("плавны старт источника")
                    if self.soft_start(number_of_channel = ch.get_number(), repeat=3) == False:
                        '''ошибка плавного старта'''
                        return ch_response_to_step.Step_fail, parameters, time.time() - start_time
                ...
                while i-1 < repeat and answer == ch_response_to_step.Step_fail:
                    time.sleep(0.2)
                    i += 1
                    if (self._set_voltage(ch.get_number(),ch.steps_voltage[ch.step_index]) == True):
                    ...
                    if (self._set_current(ch.get_number(), ch.steps_current[ch.step_index]) == True):
                        answer = ch_response_to_step.Step_done
                    else:
                        answer = ch_response_to_step.Step_fail
                        if self.is_debug:
