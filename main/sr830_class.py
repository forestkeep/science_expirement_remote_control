...
class sr830Class(base_device):
    def __init__(self, name, installation_class) -> None:
        ...
        self.setting_window.SYNK_enter.setStyleSheet(ready_style_border)
        self.setting_window.filters_enter.setStyleSheet(ready_style_border)
        self.setting_window.reserve_enter.setStyleSheet(ready_style_border)
        self.setting_window.connect_ch_enter.setStyleSheet(ready_style_border)
        self.setting_window.Filt_slope_enter_level.setStyleSheet(ready_style_border)
        self.setting_window.input_type_enter.setStyleSheet(ready_style_border)
        self.setting_window.sensitivity_enter_decimal_factor.setStyleSheet(ready_style_border)
        self.setting_window.sensitivity_enter_factor.setStyleSheet(ready_style_border)
        self.setting_window.sensitivity_enter_number.setStyleSheet(ready_style_border)
        self.setting_window.time_const_enter_decimal_factor.setStyleSheet(ready_style_border)
        self.setting_window.time_const_enter_number.setStyleSheet(ready_style_border)
        self.setting_window.time_const_enter_factor.setStyleSheet(ready_style_border)
        self.setting_window.input_channels_enter.setStyleSheet(ready_style_border)
        #==========end==========

        #===Установка слотов для сигналов===
        self.setting_window.sensitivity_enter_number.currentIndexChanged.connect(lambda: self._is_correct_parameters())
        self.setting_window.sensitivity_enter_factor.currentIndexChanged.connect(lambda: self._is_correct_parameters())
        self.setting_window.sensitivity_enter_decimal_factor.currentIndexChanged.connect(lambda: self._is_correct_parameters())
        self.setting_window.amplitude_enter.currentIndexChanged.connect(lambda: self._is_correct_parameters())
        self.setting_window.frequency_enter.currentIndexChanged.connect(lambda: self._is_correct_parameters())
        self.setting_window.amplitude_enter.currentTextChanged.connect(lambda: self._is_correct_parameters())
    ...
    def show_setting_window(self, number_of_channel):
            ...
            self.setting_window.frequency_enter.setCurrentText(str(self.active_channel.dict_buf_parameters["frequency"]))
            self.setting_window.amplitude_enter.setCurrentText(str(self.active_channel.dict_buf_parameters["amplitude"]))
            self.setting_window.sourse_meas_enter.setCurrentText(self.active_channel.dict_buf_parameters["sourse/time"])
            ...
            self.setting_window.input_channels_enter.setCurrentText(self.active_channel.dict_buf_parameters["input_channel"])
            self.setting_window.input_type_enter.setCurrentText(self.active_channel.dict_buf_parameters["input_type"])
            self.setting_window.connect_ch_enter.setCurrentText(self.active_channel.dict_buf_parameters["input_connect"])
            self.setting_window.reserve_enter.setCurrentText(self.active_channel.dict_buf_parameters["reserve"])
            self.setting_window.filters_enter.setCurrentText(self.active_channel.dict_buf_parameters["filters"])
            #==========end==========
    ...
    def _is_correct_parameters(self) -> bool:
        if self.key_to_signal_func:
            ...
            if not is_number_correct:
                self.setting_window.sensitivity_enter_number.setStyleSheet(not_ready_style_border)
            if not is_fact_correct:
                self.setting_window.sensitivity_enter_factor.setStyleSheet(not_ready_style_border)
            if not is_dec_fact_correct:
                self.setting_window.sensitivity_enter_decimal_factor.setStyleSheet(not_ready_style_border)

            if not is_freq_correct:
                self.setting_window.frequency_enter.setStyleSheet(not_ready_style_border)
            if not is_ampl_correct:
                self.setting_window.amplitude_enter.setStyleSheet(not_ready_style_border)
            #==========end==========

                #===Вернуть ответ на основе проверки параметров
            return is_number_correct and is_fact_correct and is_dec_fact_correct and is_ampl_correct and is_freq_correct
                #==========end==========
    ...
    def calculate_time_const(self) -> str:
        ...
        time_const_enter_decimal_factor = {"ks": 1000, "s": 1, "ms": 0.001, "us": 0.000001}
    ...
    def calculate_sensitivity(self) -> str:
        ...
        sensitivity_enter_decimal_factor = {"V/uA": 1, "mV/nA": 0.001, "uV/pA": 0.000001, "nV/fA": 0.000000001}  # в вольтах подсчет
    ...
    def get_parts_sen(self, value):
        sensitivity_enter_decimal_factor = {1:"V/uA", 0.001:"mV/nA", 0.000001:"uV/pA", 0.000000001:"nV/fA"} 
        numbers = [1,3,5]
        factors  = [1,10,100]
    ...
    def get_parts_time_const(self, value):
        time_const_enter_decimal_factor = {1000:"ks", 1:"s", 0.001:"ms", 0.000001:"us"}
        numbers = [1,3]
        factors  = [1,10,100]
    ...
    def add_parameters_from_window(self):
        ...
        if self.key_to_signal_func:
            ...
            self.active_channel.dict_buf_parameters["trigger"] = self.setting_window.triger_meas_enter.currentText()
            self.active_channel.dict_buf_parameters["sourse/time"] = self.setting_window.sourse_meas_enter.currentText()
            #==========end==========
    ...
    def confirm_parameters(self):
        if True:
            ...
            self.get_sin = self.sin_wave(freq=1, amplitude=1, phase_shift=0, sample_rate=100)  # Частота 1 Гц, амплитуда 1, фазовый сдвиг 0, частота дискретизации 100 Гц
        ...
        if ch.get_type() == "meas":
            ...
            if not self.is_debug:
                while not is_stop_analyze:
                    ...
                    buf_display_value = []
                    for i in range(6):
                        disp2 = self.command.get_parameter(self.command.COMM_DISPLAY, timeout=0.5, param=2)
    ...
    def check_connect(self) -> bool:
        ...
        status = self.command.get_parameter(self.command.COMM_ID, timeout=1)
        #==========end==========
